...
func main() {
	...
	var (
		dbpath = flag.String("db", "./backupdata", "directory path to database")
	)
	flag.Parse()
	args := flag.Args()
	if len(args) < 1 {
		fatalErr = errors.New("error: specify commmand")
		// after return, defered func is executed to evaluate fatalErr
		return
	}
